...
def get_bed_details_by_id(db: Session, bed_id: str):
    # outer join bed with patient and medical details
    return db.query(
            BedDetails, 
            Patient, 
            MedicalDetails
            ...
            Patient,
            Patient.patient_id == BedDetails.current_patient_id
        ).outerjoin(
            MedicalDetails, 
            BedDetails.bed_id == MedicalDetails.bed_id,
        ).filter(
            BedDetails.bed_id == bed_id
        ).first()
...
#     patient_details: MedicalDetails = db.query(MedicalDetails).filter(
...
def get_all_bed_details(db: Session, ward_number: str):
    qry = db.query(BedDetails, Patient, MedicalDetails).outerjoin(
        Patient,
        Patient.patient_id == BedDetails.current_patient_id
    ).outerjoin(
        MedicalDetails,
        BedDetails.current_patient_id == MedicalDetails.patient_id
    )
...
def save_or_update_patient(db: Session, patient: Patient):
    # res: Patient = db.query(Patient).filter(
    #     Patient.patient_id == patient.patient_id).first()
    # if res is None:
    #     db.add(patient)
    #     db.commit()
    # else:
...
def update_or_add_bed_details(session: Session, bed_details: BedDetails):
    details: BedDetails = session.query(BedDetails).filter(
        BedDetails.bed_no == bed_details.bed_no).first()
    if details is None:
...
def get_patient_details(session, ward_number: str, bed_number: str):
